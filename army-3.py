...
attendance_file = 'army.csv'
header = ['Name', 'First Seen', 'Last Seen', 'Time Elapsed', 'Laps','Distance']
...
while True:
    ...
    with open(attendance_file, mode='r') as csv_file:
        ...
        for row in reader:
            attendance_dict[row['Name']] = {'First Seen': row['First Seen'], 'Last Seen': row['Last Seen'], 'Laps': float(row['Laps']),'Distance': row['Distance']}
        ...
        if True in matches:
            ...
            if name in known_persons:
                ...
                if time_since_last_seen < 0.5:
                    ...
                    attendance_dict[name]['Laps'] += 1
                    attendance_dict[name]['Distance']=ser.readline().decode().strip()
            else:
                attendance_dict[name] = {'First Seen': current_time, 'Last Seen': current_time, 'Laps': 0,'Distance':0}
            ...
            if (isfloat((num))):  #if it is float , it is true
                if(num=='' and name  not in known_persons):
                    print(num)
                    print("Alert")
                    
                elif(1<float(num)<20 and name in known_persons):
                    print(num)
                    print("Verified")

                else:
                    print(num)
                    print("Is a Soldier , Please Come Closer with Device")
                         
        # if (ser.readline().decode().strip()!=''):
        #     #num=ser.readline().decode().strip()
        #     continue
        
        # if(float(num) < 3.5 and name == "Unknown Person"):
        #             print("Alert") 
        top = top*3
        bottom = bottom*3
        right = right*3
    ...
    for name, times in attendance_dict.items():
        ...
        time_elapsed = str(last_seen_time - first_seen_time)
        times['Time Elapsed'] = time_elapsed
    with open('/Users/abby/Documents/saves/army.csv', mode='w', newline='') as csv_file:
        #header.append('Laps')
        ...
        for name, times in attendance_dict.items():
            writer.writerow({'Name': name, 'First Seen': times['First Seen'], 'Last Seen': times['Last Seen'], 'Time Elapsed': times['Time Elapsed'], 'Laps': times['Laps'],'Distance': times['Distance']})
